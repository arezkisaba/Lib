...
namespace Lib.ApiServices.Transmission.UnitTests
{
    ...
    public class MainUnitTest
    {
        ...
        [TestMethod]
        public async Task AddTorrentAsync_DeleteTorrentAsync_TestMethod()
        {
            int? torrentId = null;
            ...
            Assert.IsTrue(torrendAdded?.arguments?.torrentadded != null || torrendAdded?.arguments?.torrentduplicate != null);

            var torrents = await _transmissionService.GetTorrentsAsync();
            Assert.IsTrue(torrents != null && torrents.Any(obj => obj.id == torrentId));

            await _transmissionService.DeleteTorrentAsync(torrentId.Value);
        }
    }
}
